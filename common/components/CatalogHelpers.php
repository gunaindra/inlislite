...
class CatalogHelpers extends Catalogs
{
    ...
    public function getTandaBaca($tag,$code) {
        ...
        if($modelcard != NULL)
        {
            ...
            for ($i=0; $i < $jumlahData ; $i++) { 
                ...
                for ($k = 0; $k < count($sItem); $k++)
                {
                    ...
                    {
                        //echo'msuk Alignment right'.$sRow;
        ...
        foreach ($tagvalues as $tagcode => $tagvalue) {
            switch ($tagcode) {
                        ...
                        foreach ($tagvalue as $key => $value) {
                            if(trim(preg_replace($regexReplaceDollar, '', $value)) != '')
                            {
                                if($key==0)
                                {
                                    if($counterAuthor==1)
                                    {
                                        $result['Author'] = trim(preg_replace($regexReplaceDollar, '', $value));
                                    }else{
                                        $result['Author'] .= $delimiter.trim(preg_replace($regexReplaceDollar, '', $value));
                                    }
                                }else{
                                    $result['Author'] .= $delimiter.trim(preg_replace($regexReplaceDollar, '', $value));
                                }
                ...
                case '264':
                    ...
                    $publishmix =  explode("$",(is_array($tagvalue)) ? $tagvalue[0] : $tagvalue);
