    ...
    public function actionIndex() {
        ...
        if (Yii::$app->request->get() && $_GET['action'] === "pencarianSederhana") {              
            ...
            if (!isset($_GET['page']) || (!isset($_SESSION['countSearch']))) {
                $command = Yii::$app->db->createCommand("CALL insertTempSederhanaOpac(:keyword,:ruas,:bahan1,:fAuthor,:fPublisher,:fPublishLoc,:fPublishYear,:fSubject,:dariTGL,:sampaiTGL,'' );");
                 $command->bindValues($params);
                $command->execute();                
            } else {
                $command = Yii::$app->db->createCommand("CALL insertTempSederhanaOpac0(:keyword,:ruas,:bahan1,:limitAwal,:limit,:fAuthor,:fPublisher,:fPublishLoc,:fPublishYear,:fSubject,:dariTGL,:sampaiTGL,'' );");
