...
class KatalogController extends Controller
{
    ...
    public function actionKarantinaProses($id)
    {
        (int)$countColl = Collections::find()
         ...
         if($countColl > 0)
         {
            $this->actionFlashMessage('danger','Koleksi dengan BIBID='.$model->BIBID.' tersambung dengan data koleksi, harap pindahkan dulu data koleksi');
         }
         else if($countFileCover > 0)
         {
            $this->actionFlashMessage('danger','Koleksi dengan BIBID='.$model->BIBID.' memiliki cover, harap pindahkan dulu cover');
         }
         else if($countFiles > 0)
         {
            $this->actionFlashMessage('danger','Koleksi dengan BIBID='.$model->BIBID.' tersambung dengan data konten digital, harap pindahkan dulu data konten digital');
         }else{
                        ...
                        if($modelq2->save())
                        {
                            ...
                            if($command->execute())
                            {
                                ...
                                if($command->execute())
                                {
                                    ...
                                    {
                                        $trans->commit(); 
                                        $this->actionFlashMessage('info','Katalog dengan BIBID='.$model->BIBID.' berhasil dikarantina.');
                                        return $this->redirect(['index']);
                                    }else{
                                        $this->actionFlashMessage('danger','Katalog dengan BIBID='.$model->BIBID.' gagal dihapus.');
                                    }
                                }
                            }
                            
                        }else{
                            $this->actionFlashMessage('danger','Katalog dengan BIBID='.$model->BIBID.' gagal mengubah timestamp di koleksi');
                        }
                    }else{
                        $this->actionFlashMessage('danger','Katalog dengan BIBID='.$model->BIBID.' Gagal disimpan di Karantina koleksi'.var_dump($modelq->getErrors()));
                    }
            ...
            try {
                ...
                if($model->validate())
                {
                    if ($model->file->saveAs($filepath)) {
                        ...
                        echo 'Gagal mengunggah berkas ke server';
                    }
                }else{
                        $error='';
                        if($model->getErrors()){
    ...
    public function actionGetMessageCheckboxProcess($success,$value,$karantinaMsg='')
    {
        if($success == true)
            if($karantinaMsg != '')
                return '<span style="color:green">'.$this->actionGetDatetimeNowStr().' - '.$karantinaMsg.' pada BIBID = '.$value.'</span><br>';
            else
                return '<span style="color:green">'.$this->actionGetDatetimeNowStr().' - Berhasil diubah  BIBID = '.$value.'</span><br>';
        else
            if($karantinaMsg != '')
                return '<span style="color:red">'.$this->actionGetDatetimeNowStr().' - '.$karantinaMsg.' BIBID = '.$value.'</span><br>'; 
            else
                return '<span style="color:red">'.$this->actionGetDatetimeNowStr().' - Gagal diubah pada BIBID = '.$value.'</span><br>'; 
    }
            ...
            switch ($post['action']) {
                ...
                case 'OPAC0':
                 ...
                 case 'KERANJANG1':
                    foreach ($rowid as $key => $value) {
                        ...
                        {
                            $msg .= $this->actionGetMessageCheckboxProcess(true,$model->BIBID,'Katalog berhasil dimasukan ke keranjang');
                        }else{
                            $msg .= $this->actionGetMessageCheckboxProcess(false,$model->BIBID,'Katalog gagal dimasukan ke keranjang');
                        }
                    }
                    break;
                ...
                case 'KARANTINA':
                    foreach ($rowid as $key => $id) {
                         ...
                         if($countColl > 0)
                         {
                            $msg .= $this->actionGetMessageCheckboxProcess(false,$model->BIBID,'Katalog tersambung dengan data koleksi, harap pindahkan dulu data koleksi');
                         }
                         else if($countFileCover > 0)
                         {
                            $msg .= $this->actionGetMessageCheckboxProcess(false,$model->BIBID,'Katalog memiliki cover, harap pindahkan dulu cover');
                         }
                         else if($countFiles > 0)
                         {
                            $msg .= $this->actionGetMessageCheckboxProcess(false,$model->BIBID,'Katalog tersambung dengan data konten digital, harap pindahkan dulu data konten digital');
                         }else{
                            $trans = Yii::$app->db->beginTransaction();
                            try {
                                $modelq1 = QuarantinedCatalogs::findOne($id);
                                        ...
                                        if($modelq2->save())
                                        {
                                            ...
                                            if($command->execute())
                                            {
                                                ...
                                                if($command->execute())
                                                {
                                                    ...
                                                    {
                                                        $trans->commit(); 
                                                        $msg .= $this->actionGetMessageCheckboxProcess(true,$model->BIBID,'Katalog berhasil dikarantina.');
                                                    }else{
                                                        $msg .= $this->actionGetMessageCheckboxProcess(false,$model->BIBID,'Katalog gagal dihapus.');
                                                    }
                                                }
                                            }
                                            
                                        }else{
                                            $msg .= $this->actionGetMessageCheckboxProcess(false,'Katalog gagal mengubah timestamp di koleksi');
                                        }
                                    }else{
                                        $msg .= $this->actionGetMessageCheckboxProcess(false,'Katalog gagal disimpan di Karantina koleksi'.var_dump($modelq->getErrors()));
                                    }
    ...
    public function actionRestore($id)
    {
        ...
        try {
            ...
            if($command->execute())
            {
                    ...
                    if($command->execute())
                    {
                        ...
                        if($command->execute())
                        {
                            ...
                            if($model->delete())
                            {
                                ...
                                $this->actionFlashMessage('danger','Data gagal direstore');
                            }
                        }
                    }
                ...
                $this->actionFlashMessage('danger','Gagal menambah data di katalog');
            }
    ...
    public function actionSave($post,$modelcat,$model,$modelbib,$for)
    {
        ...
        try {
            ...
            if($pilihjudul && $for=='coll')
            {
                ...
                $this->actionSaveCollection($model,$modelcat,$lastCatalogId,$Input_JumlahEksemplar,$trans,$for,$seq,$rda,$referUrl);
            }else{
                if($modelcat->save())
                {
    ...
    public function actionValidateRequiredSimpleForm($rda) {
        ...
        $required=array();
            ...
            $CatalogInputTags['260'] = array('260a'=>'Lokasi Terbit','260b'=>'Penerbit','260c'=>'Tahun Terbit');
            ...
            $CatalogInputTags['300'] = array('300a'=>'Jumlah Halaman','300b'=>'Ket. Ilustrasi','300c'=>'Dimensi');
        ...
        foreach ($CatalogInputTags as $key => $value) {
            ...
            if($fields)
            {
                //jika punya sub array kaya tag 260,300
                if(is_array($value))
                {
                    foreach ($value as $key2 => $value2) {
                       if ($fields->Mandatory) {
                            $required[$key2]=array('status'=>'required','message'=>$value2.' tidak boleh kosong.');
                        }else{
                            $required[$key2]=array('status'=>'','message'=>'');
                        } 
                    }
                    
                }else{
                    if ($fields->Mandatory) {
                        $required[$key]=array('status'=>'required','message'=>$value.' tidak boleh kosong.');
                    }else{
            ...
            else if($rda_req!='1' && $rda_req!='0')
            {
                ...
                if($for_req=='coll' && $isUserHasAccess == false)
                {
                    $this->getView()->registerJs('
                        swal({
                            title: " ",
                            text: "User '. Yii::$app->user->identity->username.' tidak mempunyai akses melakukan entri koleksi!",
                            type: "warning",
                            timer: 5000,
    ...
    public function actionCreateTaglistClean($taglist,$worksheetId,$for,$rda,$salincatalog=false)
    {
        ...
        foreach ($modelwf as $data) {
            ...
            //jika non rda tapi ada tag 264,336,337, atau 338 maka di skip
            if($rda == '0' && ($data->field->Tag == '264' || $data->field->Tag == '336' || $data->field->Tag == '337' || $data->field->Tag == '338'))
            {
                continue;
            }
            ...
            if (array_key_exists('inputvalue', $taglist)) 
            {
                ...
                if (!array_key_exists($data->field->Tag, $taglist['inputvalue'])) {
                    if($data->field->Fixed)
                    {
                        ...
                        $taglist['inputvalue'][$data->field->Tag] = '$a ';
                    }
    ...
    public function actionCreateTaglistSimple($model,$modelbib,$post,$for,$worksheetid,$rda)
    {
        ...
        if($modelbib->PublishLocation || $modelbib->Publisher || $modelbib->PublishYear){
            ...
            if($modelfieldPublication->Repeatable)
            {
                ...
                if($modelbib->PublishLocation)
                    $publishAll['a'] = trim($modelbib->PublishLocation);
                if($modelbib->Publisher)
                    $publishAll['b'] = trim($modelbib->Publisher);
                if($modelbib->PublishYear)
                    $publishAll['c'] = trim($modelbib->PublishYear);
                $tagsRepeatable[$tagdestination][] = $publishAll;
                $tagsRepeatableInd[$tagdestination][] = array('ind1'=>'#','ind2'=>'#');
            }else{
                if($modelbib->PublishLocation)
        ...
        if($modelbib->JenisIsi)
        {
            ...
            {
                $tagsRepeatable[336][] = array('a'=>trim($modelbib->JenisIsi));
        ...
        if($modelbib->JenisMedia)
        {
            ...
            {
                $tagsRepeatable[337][] = array('a'=>trim($modelbib->JenisMedia));
        ...
        if($modelbib->JenisCarrier)
        {
            ...
            {
                $tagsRepeatable[338][] = array('a'=>trim($modelbib->JenisCarrier));
       ...
       /* echo '<pre>'; print_r($tagsSingleInd); echo '<pre/>';
        ...
        //echo '<pre>'; print_r($tagsSingle); echo '<pre/>';
        //die;
        ...
        foreach ($modelwf as $data) {
            ...
            //jika form non rda tapi tag 264,336,337, dan 338 ada maka di skip
            if($rda == '0' && ($data->field->Tag == '264' || $data->field->Tag == '336' || $data->field->Tag == '337' || $data->field->Tag == '338'))
            {
                continue;
            }
    ...
    public function actionCreateTaglistFromCatalog($CatalogId,$Worksheet_id,$for,&$taglist,$rda)
    {
        ...
        foreach ($modelwf as $data) {
            ...
            //jika form non rda tapi tag 264,336,337 atau 338 maka di skip
            if($rda == '0' && ($data->field->Tag == '264' || $data->field->Tag == '336' || $data->field->Tag == '337' || $data->field->Tag == '338'))
            {
                continue;
            }
    ...
    public function actionCheckDuplicate()
    {
        if(Yii::$app->request->post())
        {
            ...
            $command = Yii::$app->db->createCommand("SELECT ID FROM catalogs".$extraNotID." ORDER BY ID DESC");
            /*echo "SELECT ID FROM catalogs".$extraNotID." ORDER BY ID DESC"; */
            $dataCatalogs = $command->queryAll();

            foreach ($dataCatalogs as $row) {
                $CatalogId=$row['ID'];
                $command = Yii::$app->db->createCommand("SELECT COUNT(1) AS Jumlah FROM catalog_ruas cr LEFT JOIN catalog_subruas csr ON cr.ID=csr.RuasID AND cr.CatalogId=:CatalogId  WHERE 
                (csr.SubRuas='a' AND csr.Value=:Value245a AND cr.Tag='245') OR 
                (csr.SubRuas='a' AND csr.Value=:Value260a AND cr.Tag IN('260','264')) OR
                (csr.SubRuas='b' AND csr.Value=:Value260b AND cr.Tag IN('260','264')) OR
                (csr.SubRuas='c' AND csr.Value=:Value260c AND cr.Tag IN('260','264')) ");
                $command->bindParam(':CatalogId', $CatalogId);
                $command->bindParam(':Value245a', $t245a);
                $command->bindParam(':Value260a', $t260a);
                $command->bindParam(':Value260b', $t260b);
                $command->bindParam(':Value260c', $t260c);
                $countRecords = $command->queryScalar();

                /*echo $CatalogId.'='.$countRecords.'(SELECT COUNT(1) AS Jumlah FROM catalog_ruas cr LEFT JOIN catalog_subruas csr ON cr.ID=csr.RuasID AND cr.CatalogId='.$CatalogId.'  WHERE 
                (csr.SubRuas="a" AND csr.Value="'.$t245a.'" AND cr.Tag="245") OR 
                (csr.SubRuas="a" AND csr.Value="'.$t260a.'" AND cr.Tag IN("260","264")) OR
                (csr.SubRuas="b" AND csr.Value="'.$t260b.'" AND cr.Tag IN("260","264")) OR
                (csr.SubRuas="c" AND csr.Value="'.$t260c.'" AND cr.Tag IN("260","264")) )'.'---'; */
