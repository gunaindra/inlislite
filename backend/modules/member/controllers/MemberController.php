...
class MemberController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        ...
        $sqlSumbangan = "SELECT Jumlah,Keterangan,(SELECT COUNT(*) FROM sumbangan_koleksi WHERE sumbangan_koleksi.Sumbangan_id = sumbangan.ID) as JumlahKoleksi FROM sumbangan WHERE Member_ID = 1";
    ...
    public function actionSaveFoto($id) {
        ...
        $fileName = $id .'.jpg';
        move_uploaded_file($_FILES['webcam']['tmp_name'], $pathSave .$fileName);
        //Temp
...
//            ]
    ...
    public function actionCheckboxProcess()
    {
        ...
        if(isset($post['action']) && isset($post['row_id'])){
            ...
            if(isset($post['actionid']))
                ...
                switch (trim($post['action'])) {
                    ...
                    foreach ($rowid as $key => $value) {
                        ...
                        if($model->save(false))
                        {
                            $msg =  "Status Anggota sudah diaktifkan.";
                        }else{
                            
                            throw new \yii\web\HttpException(404, 'Status Anggota gagal diaktifkan.');    
                        }
                    }
                    ...
                    foreach ($rowid as $key => $value) {
                        ...
                        if($model->save())
                        {
                            $msg = "Data telah dimasukan ke keranjang anggota.";

                        }else{
                            
                            throw new \yii\web\HttpException(404, 'Data gagal dimasukan ke keranjang anggota.');    
                        }
                    }
                    ...
                    foreach ($rowid as $key => $value) {
                        ...
                        if($rowDeleted > 0)
                        {
                            $msg2 =  "Data telah dihapus.";
                        }else{
                            
                            throw new \yii\web\HttpException(404, 'Data gagal dihapus.');    
                        }
                    }
            ...
            throw new \yii\web\HttpException(404, 'Harap pilih anggota.');
        }
        return $msg;
    }
    ...
    public function actionUploadFotoAnggota() {
        ...
        if (isset($_FILES['image'])) {
            ...
            if ($file->saveAs(Yii::getAlias('@uploaded_files/foto_anggota/'. $id . '.jpg'))) {

                //resize original pict
                Image::getImagine()->open(Yii::getAlias('@uploaded_files/foto_anggota/'. $id . '.jpg'))
                    ->resize(new Box('400', '500'))->save(Yii::getAlias('@uploaded_files/foto_anggota/'. $id . '.jpg') , ['quality' => 90]);
    ...
    public function actionHapusFoto()
    {
        ...
        $path = Yii::getAlias('@uploaded_files/foto_anggota/'. $id . '.jpg');
