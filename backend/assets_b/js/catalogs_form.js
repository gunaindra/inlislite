...
function ValidateSimpleForm()
{
  ...
  if($("#hdnRda").val() === "1")
  {
    var tags = ["245","100","700",["260a","260b","260c"],["300a","300b","300c"],"336","337","338","250","082","084","020","022","650","520"];
    var textids = ["title","author","authoradded-0",["publishlocation","publisher","publishyear"],["jumlahhalaman","keteranganillustrasi","dimensi"],"jenisisi","jenismedia","jeniscarrier","edition","class","callnumber","isbn","issn","subject","note"];
  }else{
  ...
  for (var i = 0; i < arrayLength; i++) {
    ...
    if($.isArray(tags[i]))
    {
      ...
      for (var k = 0; k < arrayLength2; k++) {
        if($("#status-"+tags[i][k]).hasClass("required"))
        {
          if($("#"+prefixText+textids[i][k]).val().trim() === compareEmpty)
          {
              ...
              if(textfocus=="")
              {
                textfocus="#"+prefixText+textids[i][k];
              }
          }else{
              $("#status-"+tags[i][k]).removeClass().addClass("required");
              $("#error-"+tags[i][k]).html("");
          }
        }else{
          $("#status-"+tags[i][k]).removeClass();
          $("#error-"+tags[i][k]).html("");
        }
      }
    }else{
      ...
      {
        if($("#"+prefixText+textids[i]).val().trim() === compareEmpty)
        {
            ...
            if(textfocus=="")
            {
              textfocus="#"+prefixText+textids[i];
            }
        }else{
...
function ValidateDuplicateSimpleForm()
{
  ...
  var t260a = $('#collectionbiblio-publishlocation').val();
  var t260b = $('#collectionbiblio-publisher').val();
  var t260c = $('#collectionbiblio-publishyear').val();
...
function ValidationBibliografis()
{
    if($("#hdnPilihJudul").val() !== "")
    {
      ...
      if($("#modeform").val() === "1")
      {
        if($("#hdnFor").val() == 'coll' && $("#hdnCrudmode").val() == 'update')
        {
          ValidateAdvanceForm();
        }else{
