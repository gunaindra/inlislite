...
class PencarianLanjutController extends \yii\web\Controller {
    ...
    public function actionIndex() {
        ...
        if (Yii::$app->request->GET() && urldecode($_GET['action']) === "pencarianLanjut") {
            ...
            if (!isset($_GET['page'])) {
                $command = $connection->createCommand("CALL insertTempLanjutOpac('" . $bahan . "','" . $bahasa . "','" . $targetPembaca . "','" . $bentukKarya . "',:temp,'" . $fAuthor . "','" . $fPublisher . "','" . $fPublishLoc . "','" . $fPublishYear . "','" . $fSubject . "','1'); ");
                $command->bindValue(':temp', $temp);
                $command->execute();
            } else {
                $command = $connection->createCommand("CALL insertTempLanjutOpac0('" . $bahan . "','" . $bahasa . "','" . $targetPembaca . "','" . $bentukKarya . "',:temp," . $limitAwal . "," . $limit . ",'" . $fAuthor . "','" . $fPublisher . "','" . $fPublishLoc . "','" . $fPublishYear . "','" . $fSubject . "','1'); ");
                 $command->bindValue(':temp', $temp);
                $command->execute();
            }
