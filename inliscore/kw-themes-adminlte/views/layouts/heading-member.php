...
if (Yii::$app->user->isGuest) {
    ...
    $memberID = Members::find()->where(['MemberNo'=>Yii::$app->user->identity->NoAnggota])->one()->ID;

    $imgCheck = Yii::getAlias('@uploaded_files') . '/'.Yii::$app->params['pathFotoAnggota'].'/temp/' . $memberID . '.jpg';

    if (!file_exists($imgCheck)) {
        // No.Photo
             $image = Yii::$app->urlManager->createUrl('../uploaded_files/'.Yii::$app->params['pathFotoAnggota'].'/temp/nophoto.jpg?timestamp=' . rand());
